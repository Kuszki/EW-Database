...
class DatabaseDriver : public QObject
{
	...
	protected:
		...
		int insertBreakpoints(const QSet<int>& Tasks, int Mode, double Radius, const QSet<QPair<double, double>>& Predef);
	...
	public slots:
		...
		void editLabel(const QSet<int>& Items, const QString& Label,
					...
					int Posset, double setX, double setY, int Just);

		void insertPoints(const QSet<int>& Items, int Mode, double Radius, const QString& Path);

		void mergeSegments(const QSet<int>& Items, int Flags, double Diff, double Length, bool Mode);
